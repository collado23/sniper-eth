...
def g_m(leer=False, d=None):
    ...
    try:
        if leer:
            v = r.get("mem_v168_sin_btc")
            return eval(v) if v else None
        else: r.set("mem_v168_sin_btc", str(d))
    except: return None
...
def bot():
    ...
    # NUEVA LISTA DE 6 MONEDAS (Sin BTC ni ETH)
    monedas = ['SOLUSDT', 'DOGEUSDT', 'XRPUSDT', 'ADAUSDT', 'LINKUSDT', 'MATICUSDT']

    print(f"🚀 V168 - 6X CON SALTO A 15X - SIN BTC/ETH")
    ...
    while True:
        try:
            ...
            for o in ops[:]:
                ...
                # SALTO A 15X CUANDO HAY OPORTUNIDAD
                if roi > 0.25 and o['x'] == 6:
                    try:
                        c.futures_change_leverage(symbol=o['s'], leverage=15)
                        o['x'] = 15
                        print(f"🔥 SALTO A 15X EN {o['s']}")
                    except: pass
            ...
            if len(ops) < 1 and cap >= 10:
                for m in monedas:
                    ...
                    if cl[-2] > e9 and e9 > e27:
                        ...
                        # Usamos el 95% del capital a 6x
                        qty = round((cap * 0.95 * 6) / precio, 1)
            ...
            print(f"💰 REAL: ${cap:.2f} | Activas: {len(ops)} | {m}", end='\r')
